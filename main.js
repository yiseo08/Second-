...
function addTask () {
    let task = { // 필요하고 관련있는 정보를 하나로 묶어주는 역할.
        id: randomIdGenerate (), /* 각각의 아이템에게 부여하는 값. 아이템을 구분해서 check 버튼이 눌린 값에 줄을 친다 */
        taskContent: taskInput.value, // taskContent의 값을 넣을 건데, 여기에는 taskInput의 value 값 들어감. 그래서 밑에 거 지워도 됨
...
function render() /* taskList 배열에 저장된 값을 화면에 보이게 만들어주는 함수 */ {
    ...
    for (let i = 0; i < /* task - 실행되기 위해 list로 바꿔주어야 한다 */list.length; i++) /* i가 taskList 배열의 길이값만큼 오되, 한 바퀴 당 i값 1씩 증가 */ {
        ...
        <div class="task-done">${list[i].taskContent  /* -> 배열에 있는 객체가 아닌 객체 안에 있는 객체의 taskContent만 출력해야 함 */}</div>
        <div>
            <button onclick="toggleComplete('${list[i].id}')">Check</button>
            <button onclick="deleteTask('${list[i].id}')">Delete</button>
        </div>
        </div>`; 
        } else { /* isComplete가 true가 아닐 때 */
            resultHTML += `<div class = "task"> 
        <div>${list[i].taskContent  /* -> 배열에 있는 객체가 아닌 객체 안에 있는 객체의 taskContent만 출력해야 함 */}</div>
        <div>
            <button onclick="toggleComplete('${list[i].id}')">Check</button>
            <button onclick="deleteTask('${list[i].id}')">Delete</button>
        </div>
...
function toggleComplete(id) {
    ...
    for (let i = 0; i < taskList.length; i++) {
        if (taskList[i].id == id /* taskList의 i 번째 있는 아이템의 id가 지금 내가 매개변수로 받은 id와 일치한다면(check 버튼을 누르는 순간, check 버튼은 taskList의 i 번째 아이템의 id를 매개변수로 받는다.) */) {
